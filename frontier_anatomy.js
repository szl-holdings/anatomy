...
(function (root) {
  ...
  var state = { manifest:null, capabilities:null, evidence:null, tab:'overview', busy:false, refreshing:false };
  ...
  var css = [
    ...
    '.fa-actions{display:flex;flex-wrap:wrap;gap:7px;margin-top:14px}.fa-btn,.fa-link{display:inline-flex;align-items:center;justify-content:center;min-height:34px;padding:0 11px;border:1px solid #35444a;background:#10181d;color:#bdc9cb;text-decoration:none;cursor:pointer;font:800 9px/1 var(--font-m,monospace);letter-spacing:.08em;text-transform:uppercase}.fa-btn.primary{border-color:#2b8c8a;background:#12302f;color:#9ff1ec}.fa-btn:hover,.fa-link:hover,.fa-btn:focus-visible,.fa-link:focus-visible{border-color:#78d9d4;color:#fff;outline:none}.fa-btn:disabled,.fa-btn[aria-disabled="true"]{opacity:.45;cursor:wait}',
    ...
    'body.fa-open .szl-holo-rail,body.fa-open #yachay-sb,body.fa-open #nq7-launcher,body.fa-open #szl-v7-launcher,body.fa-open #fa-launch{visibility:hidden} .fa-observation-summary{margin:16px 0;color:#c8e4e1;font:12px/1.6 var(--font-m,monospace)}.fa-dep{display:block;padding:18px 0}.fa-dep-heading{display:flex;gap:12px;align-items:baseline;justify-content:space-between;flex-wrap:wrap}.fa-dep-name{margin:0}.fa-freshness{color:#a3b3b6;font:10px/1.5 var(--font-m,monospace)}.fa-dep-state{display:flex;flex-wrap:wrap;gap:9px;text-align:left;min-width:0;margin:12px 0;color:#a3b3b6;font-size:11px}.fa-dep-state>span{display:flex;align-items:center;gap:5px}.fa-badge.validated,.fa-badge.measured{border-color:#496778;color:#acd2e8;background:#0f1c25}.fa-badge.observed_degraded,.fa-badge.blocked,.fa-badge.stale,.fa-badge.unknown,.fa-badge.unvalidated{border-color:#685937;color:#e1bd71}.fa-badge.invalid_response,.fa-badge.rate_limited{border-color:#663640;color:#e37d8f}.fa-measured{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:12px 0}.fa-measured>div{padding:10px;background:#101a21;min-width:0}.fa-measured dt{color:#9baeb5;font-size:10px}.fa-measured dd{margin:5px 0 0;font-size:12px;color:#dae7ea;overflow-wrap:anywhere}.fa-origin{margin-top:14px;border:1px solid #334047;padding:0 12px}.fa-origin summary{padding:13px 0;cursor:pointer;font-size:12px;color:#c1d8db;min-height:44px}.fa-origin summary:focus-visible{outline:2px solid #78d9d4;outline-offset:2px}.fa-field-list{font-size:11px;line-height:1.55}.fa-field-list dt{color:#8a9ca5;margin-top:10px}.fa-field-list dd{margin:3px 0;overflow-wrap:anywhere;color:#c0d2d8}.fa-origin ul{padding-left:18px;color:#94a6ae;font-size:11px;line-height:1.6}.fa-observation-note{font-size:11px;line-height:1.6;color:#b3a77e}.fa-btn,.fa-link,.fa-tab,.fa-close{min-height:44px}.fa-close{min-width:44px}.fa-dep-purpose{font-size:12px;color:#a0b2b9}',
  ...
  function updateRail(){
    ...
    if(state.evidence)d=Object.assign({},d,{evidence_state:state.evidence.evidence_state}); // Verifier reachability never upgrades local structural verification.
  ...
  var OBSERVATION_CONTRACT = 'szl.anatomy-dependency-observation/v1';
  var measuredLabels = {surface:'Surface',reported_evidence_state:'Upstream evidence state',receipt_count:'Reported receipts',organ:'Observed body',source_revision:'Source revision',doctrine_state:'Doctrine state',service:'Service',receipt_verified:'Receipt verified',energy:'Energy',status:'Reported status',state:'Reported state',decision:'Decision',live:'Live observation',live_count:'Live organs',organ_count:'Organs reported',blocked:'Blocked',lambda:'Trust aggregate (advisory)',lambda_floor:'Trust floor',locked_proven:'Locked proven',locked_formula_count:'Locked formula count',surface_count:'Declared surfaces',verification_result:'Verifier result',verdict:'Verdict',schema:'Response schema'};
  function observationView(dep, now){
    dep=dep||{};
    var measured=dep.measured&&typeof dep.measured==='object'&&!Array.isArray(dep.measured)?dep.measured:{};
    var typed=dep.observation_contract===OBSERVATION_CONTRACT;
    var validated=typed&&dep.contract_validated===true&&dep.http_status!==429&&dep.contract_state==='AVAILABLE'&&/^application\/(?:[a-z0-9.+-]+\+)?json(?:;|$)/i.test(dep.content_type||'');
    var stamp=Date.parse(dep.observed_at),age=Number.isFinite(stamp)?(now-stamp)/1000:null;
    var fresh=age!==null&&age>=-5&&age<=60;
    var unknown=dep.posture_state==='UNKNOWN'||measured.state==='UNKNOWN';
    var evidence=validated&&['MEASURED','LIVE'].indexOf(dep.evidence_state)!==-1&&!unknown?dep.evidence_state:'UNAVAILABLE';
    if(evidence==='LIVE'){var organs=Array.isArray(measured.organs)?measured.organs:[];var validOrgans=organs.every(function(o){return o&&typeof o==='object'&&!Array.isArray(o)&&typeof o.id==='string'&&o.id&&o.status==='LIVE';});var unique=new Set(validOrgans?organs.map(function(o){return o.id;}):[]);if(!validOrgans||dep.posture_state!=='OBSERVED_LIVE'||measured.state!=='LIVE'||measured.live!==true||measured.blocked!==false||!Number.isInteger(measured.live_count)||measured.live_count<=0||measured.live_count!==measured.organ_count||organs.length!==measured.organ_count||unique.size!==organs.length)evidence='UNAVAILABLE';}
    return {typed:typed,validated:validated,measured:measured,evidence:evidence,fresh:fresh,age:age,
      freshness:age===null?'Time unavailable':age < -5?'Clock mismatch':fresh?'Observed '+Math.max(0,Math.round(age))+'s ago':'Stale observation',
      posture:typed?String(dep.posture_state||'UNKNOWN'):'UNKNOWN'};
  }
  function measuredHTML(dep, view){
    if(!view.validated)return '<p class="fa-observation-note">No validated measurement. A response or HTTP status is not organ health.</p>';
    var values=Object.keys(measuredLabels).filter(function(key){var v=view.measured[key];return Object.prototype.hasOwnProperty.call(view.measured,key)&&(v===null||['string','number','boolean'].indexOf(typeof v)!==-1);});
    var html=values.map(function(key){var value=view.measured[key];return '<div><dt>'+esc(measuredLabels[key])+'</dt><dd>'+esc(value===null?'Unknown':String(value).slice(0,160))+'</dd></div>';}).join('');
    var organs=Array.isArray(view.measured.organs)?view.measured.organs:[];
    html+=organs.slice(0,8).filter(function(organ){return organ&&typeof organ==='object'&&!Array.isArray(organ);}).map(function(organ){return '<div><dt>'+esc(String(organ.id||organ.name||'Organ').slice(0,80))+'</dt><dd>'+esc(String(organ.status||organ.state||'UNKNOWN').slice(0,80))+'</dd></div>';}).join('');
    return '<dl class="fa-measured">'+html+'</dl>'+(view.posture==='UNKNOWN'?'<p class="fa-observation-note">The contract answered, but organ posture is unknown. Empty organs do not establish a functioning body.</p>':'')+(view.measured.verification_result==='REJECTED_INVALID_INPUT'?'<p class="fa-observation-note">The empty test input was rejected. No real receipt was verified.</p>':'');
  }
  function observationHTML(e, now, refreshing){
    e=e||{};var rows=Array.isArray(e.dependencies)?e.dependencies:[];
    var views=rows.map(function(dep){return observationView(dep,now);});
    var validated=views.filter(function(v){return v.validated;}).length;
    var measured=views.filter(function(v){return v.fresh&&v.evidence!=='UNAVAILABLE';}).length;
    return '<section class="fa-section">'+sectionHead('Body observations')+'<p class="fa-lede" style="font-size:12px">Inspect what each upstream contract actually reported. These are bounded observations, not proof of model quality or execution authority.</p>'+
      '<div class="fa-observation-summary" role="status" aria-live="polite" aria-atomic="true">'+esc(refreshing?'Probing dependencies; previous observations remain below.':validated+' / '+rows.length+' contracts validated · '+measured+' measured observations within 60s')+'</div>'+
      '<div class="fa-actions"><button class="fa-btn primary" id="fa-refresh"'+(refreshing?' aria-disabled="true"':'')+'>'+esc(refreshing?'Probing…':'Refresh observations')+'</button><a class="fa-link" href="/api/anatomy/v1/evidence" target="_blank" rel="noopener">Observation JSON ↗</a></div>'+
      rows.map(function(dep,i){var view=views[i];var hash=/^[a-f0-9]{64}$/.test(dep.response_sha256||'')?dep.response_sha256:'Unavailable';
        return '<article class="fa-dep"><div class="fa-dep-heading"><h4 class="fa-dep-name">'+esc(dep.id)+'</h4><span class="fa-freshness">'+esc(view.freshness)+'</span></div><p class="fa-dep-purpose">'+esc(dep.purpose)+'</p>'+
        '<div class="fa-dep-state"><span>Transport '+badge(dep.transport_state)+'</span><span>Contract '+badge(view.validated?'VALIDATED':view.typed?(dep.contract_state||'UNKNOWN'):'UNVALIDATED')+'</span><span class="fa-evidence-age">Evidence '+badge(!view.fresh&&view.evidence!=='UNAVAILABLE'?'STALE':view.evidence)+'</span><span>Posture '+badge(view.posture)+'</span></div>'+measuredHTML(dep,view)+
        '<details class="fa-origin"><summary>Origin, timing &amp; response digest</summary><dl class="fa-field-list"><dt>Endpoint</dt><dd><code>'+esc((dep.method||'GET')+' '+(dep.url||'Unknown'))+'</code></dd><dt>Observed</dt><dd><time>'+esc(dep.observed_at||'Unavailable')+'</time></dd><dt>Response</dt><dd>HTTP '+esc(dep.http_status==null?'Unavailable':dep.http_status)+' · '+esc(Number.isFinite(dep.latency_ms)?dep.latency_ms+' ms':'Latency unavailable')+' · '+esc(Number.isFinite(dep.response_bytes)?dep.response_bytes+' bytes':'Size unavailable')+'</dd><dt>Content type</dt><dd>'+esc(dep.content_type||'Unavailable')+'</dd><dt>Response SHA-256</dt><dd><code>'+esc(hash)+'</code></dd></dl><p class="fa-observation-note">Digest identifies the observed response bytes. It does not prove a signature, correctness, or current health.</p>'+list(dep.limits,false)+'</details></article>';
      }).join('')+(rows.length?'':'<p class="fa-observation-note">No dependency observations available. No organ state was inferred.</p>')+'</section><section class="fa-section">'+sectionHead('Interpretation limits')+list(e.limits,false)+'<div class="fa-actions"><a class="fa-link" href="https://huggingface.co/spaces/SZLHOLDINGS/governed-receipt-verifier" target="_blank" rel="noopener">Independent verifier ↗</a></div></section>';
  }
  function evidenceHTML(){
    return observationHTML(state.evidence,Date.now(),state.refreshing);
  }
  function updateObservationFreshness(host, evidence, now, refreshing){
    if(!host)return;
    var rows=evidence&&Array.isArray(evidence.dependencies)?evidence.dependencies:[];
    var views=rows.map(function(dep){return observationView(dep,now);});
    var summary=host.querySelector('.fa-observation-summary');
    if(summary){var text=refreshing?'Probing dependencies; previous observations remain below.':views.filter(function(v){return v.validated;}).length+' / '+rows.length+' contracts validated · '+views.filter(function(v){return v.fresh&&v.evidence!=='UNAVAILABLE';}).length+' measured observations within 60s';if(summary.textContent!==text)summary.textContent=text;}
    host.querySelectorAll('.fa-dep').forEach(function(node,index){var view=views[index];if(!view)return;var age=node.querySelector('.fa-freshness');if(age&&age.textContent!==view.freshness)age.textContent=view.freshness;var label=node.querySelector('.fa-evidence-age');if(label){var value='Evidence '+badge(!view.fresh&&view.evidence!=='UNAVAILABLE'?'STALE':view.evidence);if(label.innerHTML!==value)label.innerHTML=value;}});
  }
  function stopObservationClock(){if(observationTimer!==null){clearInterval(observationTimer);observationTimer=null;}}
  function startObservationClock(){
    ...
    var tick=function(){updateObservationFreshness(document.getElementById('fa-body'),state.evidence,Date.now(),state.refreshing);};
    tick();observationTimer=setInterval(tick,1000);
  }
  ...
  function restoreBodyFocus(body, focusId){
    var panel=document.getElementById('fa-panel');
    if(!body||!panel||!panel.classList.contains('open'))return;
    var replacement=focusId?document.getElementById(focusId):null;
    if(!replacement||!body.contains(replacement)||replacement.disabled||!replacement.getClientRects().length){
      replacement=document.getElementById('fa-close');
    }
  ...
  function render(){
    ...
    var ownedFocus=!!(body&&active&&body.contains(active));
    var focusId=ownedFocus?active.id:null;
    updateRail();
    if(body)body.innerHTML=bodyHTML();
    document.querySelectorAll('.fa-tab').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-tab')===state.tab);b.setAttribute('aria-selected',b.classList.contains('active')?'true':'false');});
    wireBodyActions();
    // Restore only focus invalidated by this render, never a later user's choice.
    if(ownedFocus)restoreBodyFocus(body,focusId);
  }
  ...
  function loadEvidence(force){if(state.refreshing)return;state.refreshing=true;render();fetchJSON(API+'/evidence'+(force?'?refresh=1':''),{},16000).then(function(res){state.refreshing=false;state.evidence=res.ok?res.data:{evidence_state:'UNAVAILABLE',verification_state:'UNAVAILABLE',scope:'Evidence endpoint unavailable.',summary:{live:0,total:0},dependencies:[],limits:['No dependency state was inferred.']};render();});}
  function wireBodyActions(){document.querySelectorAll('#fa-refresh').forEach(function(b){b.setAttribute('aria-disabled',state.refreshing?'true':'false');b.addEventListener('click',function(){loadEvidence(true);});});document.querySelectorAll('#fa-verify-bundle').forEach(function(b){b.addEventListener('click',verifyBundle);});}
  function open(){render();previousFocus=document.activeElement;document.body.classList.add('fa-open');document.getElementById('fa-panel').inert=false;document.getElementById('fa-panel').classList.add('open');document.getElementById('fa-panel').setAttribute('aria-hidden','false');document.getElementById('fa-scrim').classList.add('open');document.getElementById('fa-launch').setAttribute('aria-expanded','true');document.getElementById('fa-close').focus();startObservationClock();}
  function close(){stopObservationClock();document.body.classList.remove('fa-open');document.getElementById('fa-panel').inert=true;document.getElementById('fa-panel').classList.remove('open');document.getElementById('fa-panel').setAttribute('aria-hidden','true');document.getElementById('fa-scrim').classList.remove('open');document.getElementById('fa-launch').setAttribute('aria-expanded','false');if(previousFocus&&previousFocus.focus)previousFocus.focus();}
  function trapDialogTab(panel,e){
    if(e.key!=='Tab'||!panel||!panel.classList.contains('open'))return;
    var nodes=Array.from(panel.querySelectorAll('button:not([disabled]),a[href],summary,input,[tabindex="0"]')).filter(function(n){return !n.disabled&&n.tabIndex>=0&&n.getClientRects().length;});
    var first=nodes[0],last=nodes[nodes.length-1];
    if(!first){e.preventDefault();panel.setAttribute('tabindex','-1');panel.focus();return;}
    if(nodes.indexOf(document.activeElement)===-1){e.preventDefault();(e.shiftKey?last:first).focus();}
  ...
  function mount(){
    ...
    document.addEventListener('visibilitychange',function(){if(document.hidden)stopObservationClock();else startObservationClock();});
  }
  if(typeof module==='object'&&module.exports){module.exports={observationView:observationView,observationHTML:observationHTML,updateObservationFreshness:updateObservationFreshness,restoreBodyFocus:restoreBodyFocus,trapDialogTab:trapDialogTab};return;}
